...
public class Production {
    ...
    public ArrayList<Integer> computeFirstSetOfBetaAndC() {
        ...
        for (int i = dotPos + 1; i < right.size(); i++) {  // . 后面的 符号， 所以 dotPos + 1
            set.add(right.get(i));
        }
        set.addAll(lookAhead);  // 随意要吧 点后面 的所有元素 + 本身表达式的 lookahead 放在 一个队列中，一个个判断
...
// 不为 null 就跳出， 为 null 就一直 加 firstSet 的元素进去  lookahead 中
            // 算法比较死板，认真阅读下就 懂
